...
while (<$atcg>) {
	...
	$tumordel{$tmp[1]}{c}=$tumordel{$tmp[1]}{c}-($hostRN/2) if (exists $tumordel{$tmp[1]});
        ...
        if ($sum != 0) {
            for my $allele (sort keys %tumor_rawRN) {
	if ($tumor_rawRN{$allele}/$sum*$realcnv<0.5 and $tumor_rawRN{$allele}/$sum*$realcnv >= 0.3 and $tumor_rawRN{$allele} >= 1) {
	    $tumor_rawRN{$allele}=1;
	}elsif ($tumor_rawRN{$allele}/$sum*$realcnv>=0.5 and $tumor_rawRN{$allele} >= 1) {
