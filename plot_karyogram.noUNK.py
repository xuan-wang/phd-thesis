...
for line in bed_a:
    line = line.strip().split()
    try:
      plot_rects(line[3], int(line[0]), line[1],line[2], 'A', pop_order, colors, chrX)
    except ValueError: #flexibility for chrX
      plot_rects(line[3], 39, line[1],line[2], 'A', pop_order, colors, chrX)
for line in bed_b:
    line = line.strip().split()
    try:
      plot_rects(line[3], int(line[0]), line[1],line[2], 'B', pop_order, colors, chrX)
    except ValueError: #flexibility for chrX
      plot_rects(line[3], 39, line[1],line[2], 'B', pop_order, colors, chrX)

#write a legend
p = []
for i in range(len(pop_order)):
    p.append(plt.Rectangle((0, 0), 1, 1, color=colors[i]))
#p.append(plt.Rectangle((0, 0), 1, 1, color=(0.7,0.7,0.7)))
labs = list(pop_order)
#labs.append('UNK')
